(function () {
  ...
  function renderVolunteerCard(item, includeAdminControls) {
    ...
    return [
      ...
      "</div>",
      '<p class="card-meta">' + escapeHtml(item.ngoGroup + " | " + item.availability) + "</p>",
      '<div class="chip-row">' + skills + (item.location ? renderChip(item.location) : "") + "</div>",
      '<p class="card-meta shared-contact-line">' + escapeHtml(item.email + (item.phone ? " | " + item.phone : "")) + "</p>",
      controls,
      "</div>"
    ].join("");
  }
  ...
  function renderDonationCard(item, includeAdminControls) {
    ...
    return [
      ...
      '<p class="card-meta shared-contact-line">' + escapeHtml(item.contactDetails || item.ownerEmail || "") + "</p>",
      controls,
      "</div>"
    ].join("");
  }
